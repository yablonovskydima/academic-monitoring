...
class StudentFeaturesRepository:
    ...
    def get_grade_stats(self, student_ids: list[int]) -> list[dict]:
        query = (
            select(
                Grade.student_id,
                func.avg(Grade.score).label("avg_grade_overall"),
    ...
    def get_grade_trend(self, student_ids: list[int]) -> list[dict]:
        query = (
            select(
                Grade.student_id,
            ...
            .where(Grade.student_id.in_(student_ids))
            .group_by(Grade.student_id, Semester.id, Semester.academic_year, Semester.term)
            .order_by(Grade.student_id, Semester.start_date)
        )
        return [dict(row._mapping) for row in self.session.execute(query)]

    def get_late_submissions(self, student_ids: list[int]) -> list[dict]:
        query = (
            select(
                Grade.student_id,
    ...
    def get_attendance_stats(self, student_ids: list[int]) -> list[dict]:
        query = (
            select(
    ...
    def get_attendance_by_session_type(self, student_ids: list[int], session_type: SessionType) -> list[dict]:
        query = (
            select(
                Attendance.student_id,
    ...
    def get_expected_gradable_sessions(self, student_ids: list[int]) -> list[dict]:
        query = (
            select(
    ...
    def get_graded_sessions_count(self, student_ids: list[int]) -> list[dict]:
        query = (
            select(
                Grade.student_id,
